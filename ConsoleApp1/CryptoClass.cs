        ...
        public bool Form_LoadTrue()
        {
            ...
            if (File.Exists("keyfile.dat"))
            {
                if (!DecodeKey(number, "keyfile.dat"))
                {
                    ...
                    //                "Регистрация");
                    Console.WriteLine("Файл ключа не верный!");
            ...
            else
            {
                ...
                //                "Сообщите их разработчику для получения файла ключа!",
                //                "Регистрация");
                Console.WriteLine("Файл ключа отсутствует!");
                Logger.WriteLog(number, 0, "res == 0");
                try
                {
                    Clipboard.SetText(number);
                ...
                catch (Exception ex)
                {
                    ...
                    //       );
                    Console.WriteLine("Ошибка записи в буфер");
                    Logger.WriteLog(ex.Message, 0, "Form_LoadTrue");
                }
